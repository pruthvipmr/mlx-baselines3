...
class TestTD3:
    ...
    def test_td3_parameter_count(self):
        ...
        actor_params = [
            ...
            if "actor_net" in name and "target" not in name
        ]
        critic_params = [
            name for name in params.keys() if "q_net_" in name and "target" not in name
        ]
        target_actor_params = [name for name in params.keys() if "actor_target" in name]
        target_critic_params = [
            name for name in params.keys() if "q_net_target_" in name
        ]
...
class TestTD3Integration:
    ...
    def test_td3_pendulum_learning(self):
        ...
        env = gym.make("Pendulum-v1")
        model = TD3("MlpPolicy", env, learning_starts=100, batch_size=64, verbose=0)
