...
class PerformanceBenchmark:
    ...
    def time_function(
        self, func, *args, n_runs: int = 10, warmup: int = 2, **kwargs
    ) -> Dict[str, float]:
        ...
        for _ in range(warmup):
            ...
            if isinstance(result, (list, tuple)) and any(
                isinstance(x, mx.array) for x in result
            ):
    ...
    def compare_implementations(
        self, baseline_func, optimized_func, *args, **kwargs
    ) -> Dict[str, Any]:
...
class OptimizerBenchmark(PerformanceBenchmark):
    ...
    def setup_test_data(self):
        ...
        self.params = {
            "mlp_extractor.policy_net.0.weight": mx.random.normal(
                (64, 4), dtype=mx.float32
            ),
            "mlp_extractor.policy_net.0.bias": mx.random.normal(
                (64,), dtype=mx.float32
            ),
            "mlp_extractor.policy_net.2.weight": mx.random.normal(
                (64, 64), dtype=mx.float32
            ),
            "mlp_extractor.policy_net.2.bias": mx.random.normal(
                (64,), dtype=mx.float32
            ),
        ...
        self.grads = {
            k: mx.random.normal(v.shape, dtype=mx.float32)
...
class PPOUpdateBenchmark(PerformanceBenchmark):
    ...
    def setup_ppo(self):
        ...
        self.ppo.collect_rollouts(
            env, None, self.ppo.rollout_buffer, n_rollout_steps=self.ppo.n_steps
        )
...
def run_all_benchmarks() -> Dict[str, Any]:
    ...
    print(
        f"   Dtype promotion detected: {results['dtype']['dtype_promotion_detected']}"
    )
