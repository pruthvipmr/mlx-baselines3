...
class TestPPOPrediction:
    ...
    def test_predict_single_observation_discrete(self):
        ...
        assert isinstance(action, (int, np.integer)) or (
            hasattr(action, "shape") and action.shape == ()
        )
        ...
        assert isinstance(action_stoch, (int, np.integer)) or (
            hasattr(action_stoch, "shape") and action_stoch.shape == ()
        )
...
class TestPPOEdgeCases:
    ...
    def test_gradient_clipping(self):
        ...
        env = make_vec_env("CartPole-v1", n_envs=1)
        PPO("MlpPolicy", env, max_grad_norm=1.0, verbose=0)
