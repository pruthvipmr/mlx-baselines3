...
def test_ppo_save_load_roundtrip():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert set(initial_state.keys()) == set(loaded_state.keys()), "Parameter names should match"
        
        for name in initial_state:
            initial_param = initial_state[name]
            loaded_param = loaded_state[name]
            
            assert initial_param.shape == loaded_param.shape, f"Shape mismatch for {name}"
            assert mx.array_equal(initial_param, loaded_param), f"Values not identical for {name}"
        ...
        assert mx.array_equal(action1, action2), "Predictions should be identical after loading"
...
def test_policy_direct_save_load_roundtrip():
    ...
    for name in state1_copy:
        assert mx.array_equal(state1_copy[name], state2_new[name]), f"Round-trip failed for {name}"
    ...
    assert mx.array_equal(action1, action2), "Predictions should be identical after state copy"
