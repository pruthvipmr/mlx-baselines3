...
class GradientStabilityTest:
    ...
    def test_gradient_norm_stability(self, n_iterations: int = 20) -> Dict[str, float]:
        ...
        for i in range(n_iterations):
            ...
            for rollout_data in self.ppo.rollout_buffer.get(self.ppo.batch_size):
                ...
                def loss_fn(p):
                    ...
                    return self.ppo._optimized_compute_loss(
                        rollout_data, self.ppo.policy, 0.2, None, 0.0
                    )
        ...
        return {
            ...
            "grad_norm_cv": float(np.std(valid_grad_norms) / np.mean(valid_grad_norms))
            if np.mean(valid_grad_norms) > 0
            else float("inf"),
            ...
            "stability_score": 1.0
            / (1.0 + float(np.std(valid_grad_norms) / np.mean(valid_grad_norms)))
            if np.mean(valid_grad_norms) > 0
            else 0.0,
        }
        ...
        for i in range(n_checks):
            ...
            for rollout_data in self.ppo.rollout_buffer.get(self.ppo.batch_size):
                ...
                for key, data in rollout_data.items():
                    if (
                        isinstance(data, mx.array)
                        and self.ppo.enforce_float32
                        and data.dtype != mx.float32
                    ):
                        dtype_consistent = False
...
def run_gradient_stability_tests() -> Dict[str, Dict]:
    ...
    mean_grad_norm = results["gradient_stability"]["grad_norm_mean"]
    print(f"   Mean gradient norm: {mean_grad_norm:.4f}")
    print(f"   Gradient norm std: {results['gradient_stability']['grad_norm_std']:.4f}")
    grad_cv = results["gradient_stability"]["grad_norm_cv"]
    print(f"   Coefficient of variation: {grad_cv:.4f}")
    ...
    if results["dtype_consistency"]["unexpected_dtypes"]:
        unexpected = results["dtype_consistency"]["unexpected_dtypes"]
        print(f"   Unexpected dtypes: {unexpected}")

    # Numerical stability
    print("\n3. Numerical Stability Test")
    results["numerical_stability"] = test.test_numerical_stability()

    numerically_stable = results["numerical_stability"]["numerically_stable"]
    print(f"   Numerically stable: {numerically_stable}")
    ...
    # Overall assessment
    stable = (
        results["gradient_stability"]["stability_score"] > 0.5
        and results["dtype_consistency"]["dtype_consistent"]
        and results["numerical_stability"]["numerically_stable"]
    )

    stability_status = "PASS" if stable else "NEEDS_ATTENTION"
    print(f"Overall Stability Assessment: {stability_status}")
...
def test_gradient_stability():
    ...
    stability_score = results["gradient_stability"]["stability_score"]
    assert stability_score > 0.3, (
        f"Gradient stability score {stability_score:.3f} is too low"
    )

    # Check coefficient of variation is reasonable
    grad_norm_cv = results["gradient_stability"]["grad_norm_cv"]
    assert grad_norm_cv < 5.0, (
        f"Gradient norm coefficient of variation {grad_norm_cv:.3f} is too high"
    )
    ...
    has_nan = results["numerical_stability"]["has_nan"]
    has_inf = results["numerical_stability"]["has_inf"]
    assert results["numerical_stability"]["numerically_stable"], (
        f"Numerical instability detected: NaN={has_nan}, Inf={has_inf}"
    )
