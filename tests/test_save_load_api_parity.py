...
Tests:
1. Save persists model config, policy state dict, spaces, hyperparameters,
   optimizer state, and env_id
2. Load without env parameter works on standard envs (CartPole, Pendulum)
3. Loading twice yields identical predictions for the same observation and a
   deterministic policy seed
4. Backward compatibility: unknown keys warn and skip, they do not crash
"""
...
def test_save_persists_all_required_data():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert data["env_id"] == "CartPole-v1", (
            f"Expected env_id 'CartPole-v1', got {data['env_id']}"
        )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for obs in test_observations:
            ...
            assert mx.array_equal(action2, action3), (
                ...
                f"obs={obs}, action2={action2}, action3={action3}"
            )
        ...
        for key in list(state2.keys())[:3]:  # Check first 3 parameters
            assert mx.array_equal(state2[key], state3[key]), (
                f"Parameter {key} differs between loads"
            )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            unknown_key_warnings = [
                msg for msg in warning_messages if "Unknown keys" in msg
            ]
            assert len(unknown_key_warnings) > 0, (
                "Should generate warning about unknown keys"
            )
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for key in original_opt_state["m"]:
            assert mx.array_equal(
                model2.optimizer_state["m"][key], original_opt_state["m"][key]
            ), f"First moment {key} not restored correctly"

        for key in original_opt_state["v"]:
            assert mx.array_equal(
                model2.optimizer_state["v"][key], original_opt_state["v"][key]
...
def test_env_creation_fallback():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            warning_messages = [str(warning.message) for warning in w]
            fallback_warnings = [
                msg for msg in warning_messages if "No env_id found" in msg
            ]
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            failed_warnings = [
                ...
                if "Failed to recreate environment" in msg
            ]
            assert len(failed_warnings) > 0, (
                "Should warn about failed environment creation"
            )
