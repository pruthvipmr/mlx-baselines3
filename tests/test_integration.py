...
class TestIntegrationLearning:
    ...
    def test_sac_integration_pendulum(self):
        ...
        with tempfile.TemporaryDirectory():
            ...
            # Pendulum reward is negative; -1000 is random. Better performance
            # is closer to 0. Very lenient threshold for integration test.
            assert total_reward > -1500, (
                f"Total reward {total_reward} too low for basic functionality"
            )
...
class TestCallbackIntegration:
    ...
    def test_multiple_callbacks_integration(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            model.learn(
                total_timesteps=2000, callback=[eval_callback, checkpoint_callback]
            )
            ...
            checkpoint_files = [
                f for f in os.listdir(tmpdir) if f.startswith("test_checkpoint")
            ]
