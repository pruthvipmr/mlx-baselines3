...
class VecEnv(ABC):
    ...
    ) -> Tuple[ObsType, np.ndarray, np.ndarray, List[Dict[str, Any]]]:
        """
    ...
    def getattr_depth_check(self, name: str, already_found: bool) -> Optional[str]:
        ...
        if hasattr(self, name) and already_found:
            return (
                f"Found attribute {name} in both the wrapper and the wrapped "
                "environment. Using the wrapper's version."
            )
        return None
