...
class SAC(OffPolicyAlgorithm):
    ...
    entropy reinforcement learning framework. It uses:
    ...
    def __init__(
        ...
        # Set entropy parameters before super().__init__() because _setup_model()
        # is called from there
        if ent_coef == "auto":
            # Target entropy is -dim(A) for continuous action spaces (set once
            # the env is available)
            self.target_entropy = target_entropy  # Converted to float later
    ...
    def _initialize_optimizers(self) -> None:
        ...
        for name, param in self.policy.parameters().items():
            if any(
                component in name
                for component in ["features_extractor", "actor_net", "mu", "log_std"]
            ):
        ...
        if self.ent_coef_optimizer is not None and self.log_ent_coef is not None:
            ...
            self.ent_coef_optimizer_state = self.ent_coef_optimizer.init_state(
                ent_params
            )
    ...
    def _update_target_networks(self) -> None:
        ...
        updated_target_params = polyak_update(
