...
class VecNormalize(VecEnvWrapper):
    """
    A vectorized wrapper that normalizes the observations and returns of an environment.
    
    This wrapper maintains running statistics of observations and returns to normalize
    them, which can significantly improve training stability and performance.
    
    Args:
        venv: Vectorized environment to wrap
        training: Whether to update normalization statistics during environment interactions
    ...
    def __init__(
        ...
        if self.norm_obs:
            if isinstance(self.observation_space, gym.spaces.Dict):
                ...
                self.obs_rms = RunningMeanStd(shape=self.observation_space.shape, epsilon=epsilon)
        else:
            self.obs_rms = None
        ...
        if isinstance(obs, dict):
            ...
            for key, values in obs.items():
                ...
                normalized_obs[key] = np.clip(
                    self.obs_rms[key].normalize(values),
                    -self.clip_obs,
                    self.clip_obs
                )
    ...
    def get_original_obs(self) -> ObsType:
        """
        Get the original (unnormalized) observations from the wrapped environment.
    ...
    def set_training(self, training: bool) -> None:
        """
        Set training mode. When training=False, normalization statistics are not updated.
        
        Args:
            training: Whether to update statistics during environment interactions
        """
        self.training = training
    ...
    def get_attr(self, attr_name: str, indices: Optional[List[int]] = None) -> List[Any]:
        ...
        if attr_name in ["obs_rms", "ret_rms", "training", "norm_obs", "norm_reward"]:
            ...
            return [getattr(self, attr_name)] * (len(indices) if indices else self.num_envs)
        else:
    ...
    def set_attr(self, attr_name: str, value: Any, indices: Optional[List[int]] = None) -> None:
        ...
        """
        if attr_name in ["training", "norm_obs", "norm_reward", "clip_obs", "clip_reward"]:
