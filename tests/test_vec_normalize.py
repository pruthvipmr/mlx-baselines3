...
class TestVecNormalize:
    ...
    def test_observation_normalization(self):
        ...
        for _ in range(10):
            actions = np.array(
                [venv_norm.action_space.sample() for _ in range(self.n_envs)]
            )
    ...
    def test_save_load(self):
        ...
        try:
            ...
            np.testing.assert_array_almost_equal(
                venv_norm2.obs_rms.mean, original_obs_mean
            )
            np.testing.assert_array_almost_equal(
                venv_norm2.obs_rms.var, original_obs_var
            )
            np.testing.assert_array_almost_equal(
                venv_norm2.ret_rms.mean, original_ret_mean
            )
            np.testing.assert_array_almost_equal(
                venv_norm2.ret_rms.var, original_ret_var
            )
...
class TestEnvTypeAssertions:
    ...
    def test_ppo_requires_vec_env(self):
        ...
        with pytest.raises(
            AssertionError, match="PPO requires a vectorized environment"
        ):
            PPO("MlpPolicy", env)
    ...
    def test_a2c_requires_vec_env(self):
        ...
        with pytest.raises(
            AssertionError, match="A2C requires a vectorized environment"
        ):
            A2C("MlpPolicy", env)
