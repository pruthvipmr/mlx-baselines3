...
class BaseAlgorithm(ABC):
    ...
    def predict(
        ...
        if not vectorized_env:
            obs_tensor = (
                mx.expand_dims(obs_tensor, 0)
                if isinstance(obs_tensor, mx.array)
                else {
    ...
    def save(self, path: str) -> None:
        ...
        if hasattr(self.env, "spec") and self.env.spec is not None:
            ...
            hasattr(self.env, "envs")
            and len(self.env.envs) > 0
            and hasattr(self.env.envs[0], "spec")
        ):
            # For vectorized environments, get from first env
            env_id = (
                self.env.envs[0].spec.id if self.env.envs[0].spec is not None else None
            )
        ...
        # Try to save a policy alias string instead of the instance
        # to avoid pickling closures
        policy_field: Any
        try:
    ...
    def load(
        ...
        **kwargs,
        ...
        if env is None:
            ...
            if env_id is not None:
                try:
                    ...
                    warnings.warn(
                        "Failed to recreate environment "
                        f"'{env_id}': {e}. Using CartPole-v1 as fallback.",
                        UserWarning,
                    )
                    from .vec_env import DummyVecEnv
                ...
                warnings.warn(
                    "No env_id found in saved model. Using CartPole-v1 as fallback.",
                    UserWarning,
                )
                from .vec_env import DummyVecEnv
        ...
        if (
            "policy_state" in data
            and hasattr(model, "policy")
            and hasattr(model.policy, "load_state_dict")
        ):
            try:
        ...
        try:
            ...
            warnings.warn(
                "Failed to load some algorithm-specific data: "
                f"{e}. Model may not be fully restored.",
                UserWarning,
            )
        ...
        # Get algorithm-specific keys via _get_save_data() on a temporary instance
        try:
        ...
        if unknown_keys:
            warnings.warn(
                f"Unknown keys in saved model (skipping): {unknown_keys}. "
                "This may indicate the model was saved with a newer version.",
                UserWarning,
            )
    ...
    def _is_vectorized_observation(
        ...
        if isinstance(observation, dict):
            ...
            return (
                observation[first_key].shape[0] != observation_space[first_key].shape[0]
            )
        else:
...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def __init__(
        self, *args, replay_buffer_class=None, replay_buffer_kwargs=None, **kwargs
    ):
    ...
    def _sample_action(
        ...
        if action_noise is not None and not isinstance(
