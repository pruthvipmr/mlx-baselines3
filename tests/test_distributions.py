...
class TestSquashedDiagGaussianDistribution:
    ...
    def test_log_prob_jacobian_correction(self):
        ...
        # Squashed log prob should be less due to the Jacobian correction (in
        # absolute value). For this test, we only check that the correction is
        # applied, so the result should differ.
...
class TestMultiCategoricalDistribution:
    ...
    def test_log_prob(self):
        ...
        actions = mx.array(
            [[1, 0]]
        )  # Action 1 for first component, action 0 for second
...
class TestFiniteDifferenceLogProb:
    ...
    def test_gaussian_finite_diff(self):
        ...
        expected_log_prob = -0.5 * mx.sum(
            (diff / std) ** 2 + 2 * log_std + math.log(2 * math.pi), axis=-1
        )
...
class TestMakeProbaDistribution:
    ...
    def test_continuous_action_space(self):
        ...
        assert dist.action_dim == 3
        assert (
            dist.action_space is action_space
        )  # Should store action space for clipping
    ...
    def test_unsupported_action_space(self):
        ...
        action_space = gym.spaces.Tuple(
            [gym.spaces.Discrete(2), gym.spaces.Box(-1, 1, (1,))]
        )
