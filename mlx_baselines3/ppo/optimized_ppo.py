...
from mlx_baselines3.common.utils import explained_variance
...
class OptimizedPPO(OnPolicyAlgorithm):
    ...
    def _setup_optimizer(self) -> None:
        ...
        if self.verbose >= 1:
            adapter_name = type(self.optimizer_adapter).__name__
            print(f"Optimizer adapter initialized: {adapter_name}")
    ...
    def train(self) -> None:
        ...
        clip_range_vf = (
            self._get_schedule_value(self.clip_range_vf)
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                ...
                def loss_fn(p):
                    ...
                    return self._optimized_compute_loss(
                        rollout_data, self.policy, clip_range, clip_range_vf, ent_coef
                    )
                ...
                if (
                    self.optimizer_adapter is not None
                    and self.optimizer_state is not None
                ):
                    try:
                ...
                ratio = mx.exp(log_prob - rollout_data["log_probs"])
                policy_loss_1 = advantages * ratio
                policy_loss_2 = advantages * mx.clip(
                ...
                clip_fraction = mx.mean(
                    (mx.abs(ratio - 1) > clip_range).astype(mx.float32)
                )
            ...
            if (
                ...
                if self.verbose >= 1:
                    mean_kl = float(np.mean(approx_kl_divs))
                    print(
                        "Early stopping at step "
                        f"{epoch} due to reaching max kl: {mean_kl:.2f}"
                    )
                break
    ...
    def collect_rollouts(
        self, env: VecEnv, callback, rollout_buffer: RolloutBuffer, n_rollout_steps: int
    ) -> bool:
