...
class FinalPerformanceComparison(PerformanceBenchmark):
    ...
    def setup_test_environment(self):
        ...
        self.baseline_ppo.collect_rollouts(
            env1, None, self.baseline_ppo.rollout_buffer, n_rollout_steps=self.baseline_ppo.n_steps
        )
        
        self.optimized_ppo.collect_rollouts(
            env2, None, self.optimized_ppo.rollout_buffer, n_rollout_steps=self.optimized_ppo.n_steps
        )
    ...
    def baseline_single_epoch(self):
        ...
        for rollout_data in self.baseline_ppo.rollout_buffer.get(self.baseline_ppo.batch_size):
            ...
            def loss_fn(p):
                ...
                return self.baseline_ppo._compute_loss(rollout_data, self.baseline_ppo.policy, clip_range, clip_range_vf, ent_coef)
    ...
    def optimized_single_epoch(self):
        ...
        # Train for one epoch (simplified)
        for rollout_data in self.optimized_ppo.rollout_buffer.get(self.optimized_ppo.batch_size):
            # Get current clip ranges and entropy coefficient
            clip_range = 0.2
            clip_range_vf = None
            ent_coef = 0.0
            ...
            def loss_fn(p):
                ...
                return self.optimized_ppo._optimized_compute_loss(rollout_data, self.optimized_ppo.policy, clip_range, clip_range_vf, ent_coef)
    ...
    def benchmark_single_epoch_comparison(self):
        ...
        return self.compare_implementations(
            self.baseline_single_epoch,
            self.optimized_single_epoch,
            n_runs=15
        )
...
def run_final_performance_test() -> Dict[str, Any]:
    ...
    overall_meets_target = (
        results["single_epoch"]["meets_target"] or 
        results["full_training"]["meets_target"]
    )
    ...
    print(f"  Performance target achievement: {'PASS' if overall_meets_target else 'PARTIAL'}")
    
    # JIT-specific results
    print(f"\nJIT Compilation Status:")
    if hasattr(comparison.optimized_ppo, 'jit_ops') and comparison.optimized_ppo.jit_ops:
        print(f"  JIT enabled: {comparison.optimized_ppo.jit_ops.jit_enabled}")
        if comparison.optimized_ppo.jit_ops.jit_enabled:
            print(f"  JIT optimizations active in optimized PPO")
...
def test_final_performance():
    ...
    assert worst_improvement > -50, \
        f"Worst performance regression is {worst_improvement:.1f}%, exceeding -50% threshold"
    ...
    if not overall_success:
        ...
        print("This may be due to measurement variance or the optimizations not being beneficial for this specific case")
