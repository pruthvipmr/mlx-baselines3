...
class TestA2CBasic:
    ...
    def test_hyperparameters(self):
        ...
        assert model.normalize_advantage
        assert not model.use_rms_prop
...
class TestA2CEnvironments:
    ...
    def test_continuous_action_space(self):
        ...
        assert hasattr(action[0], "__iter__") or isinstance(
            action[0], (float, np.floating, np.ndarray)
        )
        assert len(action[0]) == 1  # Pendulum has 1D action space

    @pytest.mark.skipif(
        not hasattr(gym.spaces, "MultiDiscrete"), reason="MultiDiscrete not available"
    )
    def test_multi_discrete_action_space(self):
        ...
        with pytest.raises(AssertionError, match="Action space.*is not supported"):
            A2C("MlpPolicy", env, n_steps=10, verbose=0)
...
class TestA2CPerformance:
    ...
    def test_cartpole_learning(self):
        ...
        assert avg_reward > 50, (
            f"Average reward {avg_reward} is too low for A2C on CartPole"
        )
