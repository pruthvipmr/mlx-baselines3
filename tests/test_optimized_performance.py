...
from mlx_baselines3.common.optimized_training import (
    create_optimized_ppo_trainer,
)
...
class OptimizedPPOPerformanceTest(PerformanceBenchmark):
    ...
    def setup_test_environment(self):
        ...
        self.test_batch = next(
            iter(self.baseline_ppo.rollout_buffer.get(self.baseline_ppo.batch_size))
        )
    ...
    def optimized_single_update(self):
        ...
        params, optimizer_state, step_stats = (
            self.optimized_trainer.optimized_train_step(
                params=params,
    ...
    def baseline_epoch_training(self):
        ...
        for rollout_data in self.baseline_ppo.rollout_buffer.get(
            ...
            def loss_fn(p):
                self.baseline_ppo.policy.load_state_dict(p, strict=False)
                return self.baseline_ppo._compute_loss(
                    rollout_data, self.baseline_ppo.policy, 0.2, None, 0.0
                )
    ...
    def optimized_epoch_training(self):
        ...
        # Use optimized trainer
        params, optimizer_state, epoch_stats, continue_training = (
            self.optimized_trainer.train_epoch(
                params=params,
                optimizer_state=optimizer_state,
...
def run_optimization_benchmarks() -> Dict[str, Any]:
    ...
    target_status = "PASS" if sum(total_improvements) >= 2 else "NEEDS_WORK"
