...
class A2CPolicy(ActorCriticPolicy):
    ...
    This class extends the base ActorCriticPolicy with A2C-specific
    ...
    Args:
        ...
        use_expln: Use ``expln()`` instead of ``exp()`` to ensure positive
            ...
            prevents it from growing too fast. In practice, ``exp()`` is
            usually enough.
...
def get_a2c_policy_class(policy_name: str) -> Type[A2CPolicy]:
    ...
    if policy_name in A2C_POLICY_CLASSES:
        ...
        available = list(A2C_POLICY_CLASSES.keys())
        raise ValueError(
