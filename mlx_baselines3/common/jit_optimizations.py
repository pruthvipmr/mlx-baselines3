...
@jit_loss_computation  
def jit_gradient_clipping(grads_flat: mx.array, max_norm: float) -> Tuple[mx.array, mx.array]:
    """
...
class JITOptimizedOperations:
    ...
    def optimized_ppo_loss(
        ...
        if self.jit_enabled:
            return self.compiled_ops['ppo_loss'](
                values, log_probs, old_log_probs, advantages, returns,
                old_values, entropy, clip_range, clip_range_vf, ent_coef, vf_coef
            )
        else:
    ...
    def optimized_grad_clipping(
        ...
        if self.jit_enabled:
            clipped_flat, grad_norm_array = self.compiled_ops['grad_clip'](grads_flat, max_norm)
...
def benchmark_jit_performance() -> Dict[str, Any]:
    ...
    def regular_loss():
        return jit_ppo_loss_core(
            values, log_probs, old_log_probs, advantages, returns,
            old_values, entropy, 0.2, 0.0, 0.0, 0.5
        )
    ...
    return {
        ...
        "improvement_pct": ((regular_mean - jit_mean) / regular_mean * 100) if regular_mean > 0 else 0.0
    }
