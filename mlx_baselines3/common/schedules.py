...
def get_schedule_fn(value: Union[float, str, Callable]) -> Callable:
    ...
    if callable(value):
        ...
        if value == "constant":
            ...
            # Parse linear schedule specification like "linear_0.001" or "linear_0.001_0.0"
            parts = value.split("_")
            if len(parts) == 2:
                try:
...
def schedule_from_string(schedule_str: str, default_value: float = 1.0) -> Callable[[float], float]:
    """
