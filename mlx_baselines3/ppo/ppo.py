...
leading to more stable training compared to vanilla policy gradients.
"""
                ...
                if self.logger is not None:
                    ...
                    if hasattr(self, "ep_info_buffer") and len(self.ep_info_buffer) > 0:
                        ...
                        ep_len_mean = safe_mean(
                            np.array([ep_info["l"] for ep_info in self.ep_info_buffer])
                        )
                        ep_rew_mean = safe_mean(
                            np.array([ep_info["r"] for ep_info in self.ep_info_buffer])
                        )
